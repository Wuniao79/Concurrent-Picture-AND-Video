...
export const useLanes = (options: UseLanesOptions) => {
  ...
  const resolveModelProvider = (modelId: string): ModelProvider => {
    const m = availableModels.find((item) => item.id === modelId);
  ...
  const resolveFallbackModelId = (targetProvider: ModelProvider): string | null => {
    const candidate = availableModels.find((m) =>
      targetProvider === 'gemini' ? m.provider === 'gemini' : !m.provider || m.provider === 'openai'
    );
    return candidate?.id || selectedModelId || availableModels[0]?.id || null;
  };
  ...
  useEffect(() => {
    ...
    setLanes((prev) => {
      ...
      const next = prev.map((lane) => {
        const laneProvider = resolveModelProvider(lane.model);
